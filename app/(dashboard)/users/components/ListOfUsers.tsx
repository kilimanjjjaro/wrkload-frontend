  ...
  return (
    <MasonryGrid>
      {users.map((user, index) => (
        <div key={index} className='relative flex flex-col items-start bg-white group p-7 dark:bg-white --3xl'>
          <div className='relative'>
            <Image
              ...
              height='80'
            />
            <div className='absolute top-0 left-0 w-[18px] h-[18px] --full bg-custom-red' />
          </div>
          <h3 className='mb-4 text-3xl font-bold font-primaryFont'>{user.username}</h3>
          <div className='flex mb-3 items-center h-6 px-2 text-[10px] font-bold text-gray-500 uppercase border-2 border-gray-500 --full gap-x-1 font-secondaryFont'>
            {user.role === 1 && 'Administrator'}
            {user.role === 2 && 'Project Manager'}
            {user.role === 3 && 'Partner'}
          </div>
          <div className='flex items-center h-8 px-4 mb-3 text-xs text-white bg-black --full gap-x-1 font-secondaryFont'><EnvelopeIcon className='w-4 stroke-width-2' /> {user.email}</div>
          <div className='flex items-center h-8 px-4 text-xs text-white bg-black --full gap-x-1 font-secondaryFont'><UserIcon className='w-4 stroke-width-2' /> {user.registeredAt}</div>
          <div className='absolute left-0 flex justify-center w-full transition ease-in-out opacity-100 duration-400 -top-4 xl:opacity-0 gap-x-3 group-hover:opacity-100'>
            <div className='flex items-center h-8 px-4 text-black transition ease-in-out --full cursor-pointer duration-400 bg-custom-yellow hover:text-white hover:bg-black'><PencilSquareIcon className='w-4 stroke-width-2' /></div>
