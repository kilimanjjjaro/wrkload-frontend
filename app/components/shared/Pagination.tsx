...
export default function Pagination ({ data }: { data: PaginationInterface }): JSX.Element {
  ...
  const handlePrevPage = (): void => {
    ...
    if (query !== null && pathname !== '/tasks/search') return router.push(`${pathname as string}?query=${query}&page=${data.prevPage}`)

    router.push(`${pathname as string}?page=${data.prevPage}`)
  }
  ...
  const handleNextPage = (): void => {
    ...
    if (query !== null && pathname !== '/tasks/search') return router.push(`${pathname as string}?query=${query}&page=${data.nextPage}`)

    router.push(`${pathname as string}?page=${data.nextPage}`)
  }
  ...
  return (
    <div className='flex justify-center mt-10 gap-x-5'>
      ...
      <div className='self-center text-sm text-white dark:text-white font-secondaryFont'>Page {data.page} of {totalPages}</div>
